...
class Gstr1Tool(models.Model):
    ...
    def _get_attachment_count(self):
        for gst in self:
            ...
            gst.update({'attachment_count': len(attachments)})
    ...
    def onchange(self, values, field_name, field_onchange):
        ...
        res = super(Gstr1Tool, self.with_context(context)).onchange(values, field_name,field_onchange)
        return res
    ...
    def _compute_invoice_lines(self):
        ...
        for gstr1Obj in self:
            ...
            if context.get('current_id'):
                filter = ('id', '!=', context.get('current_id'))
    ...
    def fetchInvoices(self):
        context = dict(self._context or {})
        filter = ('id', '!=', self.id)
        invoiceObjs = self.with_context(context).getInvoiceObjs(filter, 'out_invoice')
    ...
    def fetchSupplierInvoices(self):
        context = dict(self._context or {})
        filter = ('id', '!=', self.id)
        invoiceObjs = self.with_context(context).getInvoiceObjs(filter, 'in_invoice')
        self.invoice_lines = [(6, 0, invoiceObjs.ids)]
        if invoiceObjs:
            self.updateInvoiceCurrencyRate(invoiceObjs)
            self.updateGSTInvoiceLines(invoiceObjs)
            ...
            if invoiceObj.type == 'in_invoice':
                ...
                if invoiceObj.partner_id.country_id.code == 'IN':
                    if invoiceObj.partner_id.vat:
                        ...
                        if not invoiceObj.l10n_in_export_type:
                            invoiceObj.l10n_in_export_type = 'regular'
                    else:
        ...
        if invoiceObjs:
            ...
            for invoice_type, active_ids in typeDict.items():
                ...
                respData = self.env['export.csv.wizard'].with_context(
                    context).exportCsv(active_ids, invoice_type, name, gstType)
            ...
            if not self.hsn_attachment:
                respHsnData = self.env['export.csv.wizard'].with_context(
                    context).exportCsv(invoiceIds, 'hsn', name, gstType)
                if respHsnData:
