...
class gst_dashboard(models.Model):
    ...
    def _get_count(self):
        for rec in self:
            not_upload_count = len(self.env['account.move'].search([
                ('invoice_type', '=', rec.invoice_type),
                ('gst_status', '=', 'not_uploaded')
            ]))
            ready_count = len(self.env['account.move'].search([
                ('invoice_type', '=', rec.invoice_type),
                ('gst_status', '=', 'ready_to_upload')
            ]))
            uploaded_count = len(self.env['account.move'].search([
                ('invoice_type', '=', rec.invoice_type),
                ('gst_status', '=', 'uploaded')
            ]))
            filed_count = len(self.env['account.move'].search([
                ('invoice_type', '=', rec.invoice_type),
                ('gst_status', '=', 'filed')
            ]))
            rec.not_upload_count = not_upload_count
            rec.ready_count = ready_count
            rec.uploaded_count = uploaded_count
            rec.filed_count = filed_count

    def _get_amount(self):
        for rec in self:
            rec.not_upload_amount = rec.getTotalAmount('not_uploaded')
            rec.ready_amount = rec.getTotalAmount('ready_to_upload')
            rec.uploaded_amount = rec.getTotalAmount('uploaded')
            rec.filed_amount = rec.getTotalAmount('filed')
    ...
    invoice_type = fields.Selection([('b2b', 'B2B'), ('b2bur', 'B2BUR'),
                                     ('b2cl', 'B2CL'), ('b2cs', 'B2CS'),
    ...
    def action_create_new(self):
        ctx = self._context.copy()
        model = 'gstr1.tool'
        view_id = self.env.ref('slc_club_gst_tds.gstr1_tool_form').id
        name = "GST Invoice"
