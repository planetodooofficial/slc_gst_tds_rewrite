...
class Gstr1Tool(models.Model):
    ...
    def action_view_invoice(self):
        ...
        action = self.env.ref('slc_club_gst_tds.customer_invoice_list_action').read()[0]
        if len(invoices) > 1:
    ...
    def action_view_attachment(self):
        ...
        if self.json_attachment:
            attachments.append(self.json_attachment.id)
        action = self.env.ref('slc_club_gst_tds.gst_attachments_action').read()[0]
    ...
    def generateCsv(self):
        ...
        return {
            ...
            'view_id': self.env.ref('slc_club_gst_tds.message_wizard_form1').id,
            'res_id': partial.id,
